...
if not vim.fn.expand('%:t'):match('%.slides%.md$') then
  return
end
...
local get_data_and_theme = function(tbl)
  ...
  if tbl.args ~= '' then
    ...
    if vim.loop.fs_stat(cwd .. '/' .. theme) then
      data = cwd
    else
      data = std
    end
...
local marp_once = function(tbl)
  if cmd_id then
    return vim.notify('marp: marp already running.', vim.log.levels.INFO)
  end
...
local marp_stop = function()
  if not cmd_id then
    return vim.notify('marp: no process running.', vim.log.levels.INFO)
  end
...
local marp_start = function(tbl)
  ...
  cmd_id = vim.api.nvim_create_autocmd(
    { 'BufFilePre', 'BufUnload', 'VimLeavePre' },
    {
      buffer = bufnr,
      once = true,
      callback = function()
